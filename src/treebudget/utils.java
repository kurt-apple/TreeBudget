...
public class utils {
    public static boolean choicebox(String newMessage) {
        ...
        TidierFXML cbLoader = new TidierFXML("FXMLChoiceBox.fxml", Modality.APPLICATION_MODAL);
        cbLoader.getStage().showAndWait();
        return choice; }
    
    public static boolean errorbox(String newMessage) {
        msgString = newMessage;
        TidierFXML ebLoader = new TidierFXML("FXMLErrorBox.fxml", Modality.APPLICATION_MODAL);
        ebLoader.getStage().showAndWait();
        return true; }
    
    public static boolean spawn(String message, Predicate<String> action) { return action.test(message); }
    public static void closeCurrentWindow(Button b) { ((Stage) b.getScene().getWindow()).close(); }
    ...
    public static TreeTableCell<ObservableTreeBudgetItem, Double> getCurrencyTreeTableCell() {
